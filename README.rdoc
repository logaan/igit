...
any git command without prefixing it with the word git (eg. __push origin master__ rather than @@git-push origin master@@).

The working directory is set to the \<code><path></code> passed to get when loading it (defaults to '.'). It may
